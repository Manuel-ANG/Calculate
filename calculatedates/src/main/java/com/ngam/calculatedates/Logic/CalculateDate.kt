...
class CalculateDate(private val ctx: Context) {
        ...
        try {
            ...
            if (anios > 0) {
                builder.append(anios).append(
                    if (anios == 1) " ${ctx.getString(R.string.year)}" else " ${ctx.getString(
                            R.string.years
                        )
                    }"
                )
                if (meses > 0) {
                    ...
                    builder.append(meses).append(
                        if (meses > 1) " ${ctx.getString(R.string.months)}" else " ${
                            ctx.getString(
                                R.string.month
                            )
                        }"
                ...
                if (dias > 0) {
                    ...
                    builder.append(dias).append(
                        if (dias == 1) " ${ctx.getString(R.string.day)}" else " ${
                            ctx.getString(
                                R.string.days
                            )
                        }"
    ...
    fun calculateLargeDate(start: Date, now: String?): String? {
        var addeddays = 0
        var dias = 0
        val meses: Int
        val anios: Int
        ...
        try {
            val nacimiento = DateTime(start.time);
            val mAno = Large.calculateYears(nacimiento, current)
            ...
            if (anios > 0) {
                builder.append(anios).append(
                    if (anios == 1) " ${ctx.getString(R.string.year)}" else " ${
                        ctx.getString(
                            R.string.years
                        )
                    }"
    ...
    fun calculateLargeDate(start: Long, now: String?): String? {
        var addeddays = 0
        var dias = 0
        val meses: Int
        val anios: Int
